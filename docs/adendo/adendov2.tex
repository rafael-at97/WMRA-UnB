...
No uso de parâmetros Denavit-Hartenberg para descrever manipuladores robóticos, bem como para posicionar os sistemas de referência em 
relação a sua estrutura, existem duas convenções vigentes na literatura. Embora as notações sejam semelhantes, representando uma 
junta como duas translações, $a$ e $d$, e duas rotações $\alpha$ e $\theta$, as matrizes de transformação apresentam diferenças \cite{reddy2014difference}.
Para a versão clássica, utilizada por Spong \cite{spong2008robot}, $\alpha_i$ e $a_i$ são definidos com base no eixo $\hat{X}$ da junta $i$, 
já os parâmetros $d_i$ e $\theta_i$ são definidos com base no eixo $\hat{Z}$ da junta $i-1$, assim como mostra a figura \ref{fig:Spong}.

\begin{figure}[ht]
  \caption{Arranjo clássico \cite{spong2008robot}.}    
  \begin{centering}

      \includegraphics[width=0.8\columnwidth]{images/spong.png}
...
Já no modelo modificado, utilizado por Craig \cite{craig2009introduction}, utilizam-se os parâmetros $\alpha_{i-1}$ e $a_{i-1}$, definidos 
em função do eixo $\hat{X}_{i-1}$, e os parâmetros $d_i$ e $\theta_i$, definidos em função de $\hat{Z}_i$, assim como mostra a figura
\ref{fig:Craig}. 

\begin{figure}[ht]
  \caption{Arranjo modificado \cite{craig2009introduction}.}    
  \begin{centering}

      \includegraphics[width=0.65\columnwidth]{images/craig.png}
...
Utilizando a definição para o termo $d_i$ exposta no trabalho \cite{craig2009introduction} ($d_i$ significa a distância 
entre $\hat{X}_{i-1}$ e $\hat{X}_i$, ao longo de $\hat{Z}_i$), nota-se na figura
do trabalho de graduação, \ref{fig:tg}, que o parâmetro $d_6$ está ao longo do eixo $\hat{Z}_6$, seguindo a convenção proposta
por Craig. O eixo $\hat{Z}_6$, devido à construção do manipulador, coincidirá sempre com o eixo $\hat{Y}_5$ para a convenção adotada.
De acordo com a figura \ref{fig:Craig}, o parâmetro foi definido entre a origem do sistema da junta $i$, com $i=6$, e 
a intersecção entre $\hat{Z}_i$ ($\hat{Z}_6$) e $\hat{X}_{i-1}$ ($\hat{X}_5$), que coincide com a origem do sistema $\{5\}$.

\begin{figure}[ht]
  \caption{Arranjo proposto.}    
  \begin{centering}

      \includegraphics[width=1\columnwidth]{images/tg.png}
